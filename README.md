...
## 📖 О проекте
Telegram-бот, который позволяет пользователям просматривать города на географической карте. Бот сохраняет избранные города в личную коллекцию и визуализирует их на карте мира с помощью библиотек Cartopy и Matplotlib. Проект демонстрирует работу с геоданными, базами данных и географической визуализацией.

## ✨ Основные возможности
🗺️ **Показ городов** - отображение указанного города на карте мира  
⭐ **Избранные города** - сохранение городов в личную коллекцию  
...
### Структура файлов:
```
├── 📄 bot.py              # Основной файл (обработчики команд)
...
**Таблица cities** - содержит информацию о городах (название, координаты)  
**Таблица users_cities** - связывает пользователей с их избранными городами
...
| `/help` | Получить список доступных команд | `/help` |
| `/show_city <название>` | Отобразить город на карте | `/show_city London` |
| `/remember_city <название>` | Сохранить город в избранное | `/remember_city Paris` |
| `/show_my_cities` | Показать все сохраненные города на карте | `/show_my_cities` |
...
2. **👤 Пользователь →** `/show_city Moscow`
   - 🤖 Бот: Отображает карту с отмеченным городом Москва

3. **👤 Пользователь →** `/remember_city Moscow`
   - 🤖 Бот: Сохраняет город в избранное пользователя

4. **👤 Пользователь →** `/show_my_cities`
   - 🤖 Бот: Отображает карту со всеми сохраненными городами
...
- 🌍 **Расстояние между городами** - расчет и отображение расстояния
- 🎨 **Разные маркеры** - различные цвета и иконки для городов
- 📍 **Поиск по координатам** - добавление городов по широте/долготе
- 🗺️ **Разные стили карт** - выбор фонового изображения
- 📊 **Статистика** - количество сохраненных городов у пользователей
- 🔍 **Нечеткий поиск** - поиск городов по части названия
- 📱 **Инлайн-режим** - работа в любом чате
...
### Для production-версии добавьте логирование:
```python
import logging
logging.basicConfig(
    level=logging.INFO,
